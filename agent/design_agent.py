...
design_agent.py
--------------
Handles design-related tasks and assignments.
"""
...
class DesignAgent:
    ...
    def handle_task(self, task):
        ...
        for emp in self.employees:
            ...
            if not emp_skills & set(normalize_skill(s) for s in DESIGN_SKILLS):
                continue
        ...
        if best:
            explanation = f"Best skill match for required skills: {', '.join(sorted(best_overlap)) if best_overlap else 'General design expertise'}"
            return [{
